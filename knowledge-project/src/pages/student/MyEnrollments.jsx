      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-10'>
        <thead className='text-gray-900  border-gray-500/20 text-sm text-left max-sm:hidden'>
          <tr>
            ...
            <th className='px-4 py-3 font-semibold truncate'>Completé</th>
          ...
          {enrolledCourses.map((course, index)=>(
            <tr key={index} className='border-b border-gray-500/20'>
              ...
              <td className='px-4 py-3 max-sm:text-right'>
                ...
                max-sm:text-xs text-white rounded' onClick={()=> navigate('/player/' + 
                  course._id)}>

                  {progressArray[index] && progressArray[index].lectureCompleted /
                  progressArray[index].totalLectures === 1 ? 'Complété' : 'En cours' }
                  </button>
              </td>
            </tr>
          ))}
